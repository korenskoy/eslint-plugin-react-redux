...
const filterReports = (rule, getPropNameFromReactRuleMessage, getPropNameFromTeactnRuleMessage) => Object.freeze({
  create(context) {
    const removeProps = [];
        ...
        report: {
          ...
          value() {
            ...
            if (reportDescriptor.message.indexOf('exclude:') > -1) {
              removeProps.push(getPropNameFromTeactnRuleMessage(reportDescriptor.message));
            } else {
...
module.exports = (rules, getPropNameFromReactRuleMessage, getPropNameFromTeactnRuleMessage) => filterReports(ruleComposer.joinReports(rules), getPropNameFromReactRuleMessage, getPropNameFromTeactnRuleMessage);
